...
class enrolreassessmentgroupusers extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        $sql = "SELECT * FROM " . $grouptab ." WHERE group_name LIKE '%2019/20%-R'";
        if ($rs = $extdb->Execute($sql)) {
            ...
            echo 'Error reading data from the external catlevel table ($grouptab), ' . $grouptab . '<br>';
            return 4;
        ...
        foreach ($groups as $group) {
            $users = [];
            $sql = "SELECT DISTINCT student_code FROM " . $usrenrolgrouptab ." WHERE assessment_idcode = '" . $group[group_name] ."'";
            if ($rs = $extdb->Execute($sql)) {
                if (!$rs->EOF) {
                    while ($fields = $rs->FetchRow()) {
                        $fields = array_change_key_case($fields, CASE_LOWER);
                ...
                // Report error if required.
                $extdb->Close();
                echo 'Error reading data from the external catlevel table ($usrenrolgrouptab), ' . $usrenrolgrouptab . '<br>';
                return 4;
            }
